...
//set custom budget
userapp.post('/set-custom-budget/:username',verifyToken,expressAsyncHandler(async(req,res)=>{
    username=req.params.username
    mB=req.body;
    const startDate=new Date(mB.startDate)
    startDate.setUTCHours(0,0,0,0)
    const inputDate = new Date(mB.endDate);
    //End Time
    // inputDate.setHours(23, 59, 59, 999);
    inputDate.setUTCHours(23, 59, 59, 999);
    await userscollection.updateOne({username:username},{$set:{'customBudget.budget':mB.budget,'customBudget.startDate':startDate,'customBudget.endDate':inputDate}})
    res.send({message:"Budget Updated",budget:mB,username:username,endDate:inputDate})
}))
